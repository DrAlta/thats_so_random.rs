...
pub trait RandomNumberGenerator {
    ...
    fn remove_random_item<T, C: RemoveRandomItem<T, Self>>(
        &mut self,
        collection: &mut C,
    ) -> Option<T>
    where
    ...
    fn get_random_item<'a, 'b, T, C: GetRandomItem<'b, T, Self>>(
        &'a mut self,
        collection: &'b C,
    ) -> Option<T>
    where
